#!/usr/bin/env python3
"""
bughunt — CLI for structured recon, classification, triage & reporting.

Bridges the repo's skill content into a real runner. Four subcommands compose
the engagement loop:

  bughunt recon <target>           passive subdomain enum + live-host probe + URL
                               classification. Writes recon/<target>/.
  bughunt classify <url>           pattern-match a single URL against hunt-* skill
                               descriptions; print the matched skills + ranked
                               attack candidates from docs/disclosed-reports/.
  bughunt triage <finding.md>      run the triage-validation 7-Question Gate
                               against a finding file. Output: PASS / DOWNGRADE
                               / KILL with reason.
  bughunt report <finding.md>      emit a report draft (H1 / Bugcrowd / Intigriti /
                               Immunefi templates) based on finding metadata.

Stdlib + optional `requests`. No build step. Drop on PATH:

    ln -s $(pwd)/scripts/bughunt.py /usr/local/bin/bughunt

Or run inline:

    scripts/bughunt.py recon target.com
"""
...
def configure_http_proxy(proxy_url: str | None = None) -> tuple[bool, str]:
    ...
    Resolution order:
      ...
      2. BUGHUNT_BURP_PROXY env var
      3. HTTPS_PROXY / HTTP_PROXY env vars
      4. fallback: auto-detect default Burp on http://127.0.0.1:8080 (only if --burp flag)
    """
    global _HTTP_OPENER
    if not proxy_url:
        proxy_url = os.environ.get("BUGHUNT_BURP_PROXY") or os.environ.get("HTTPS_PROXY") or os.environ.get("HTTP_PROXY")
    if not proxy_url:
        _HTTP_OPENER = None
...
def recon_subdomains_via_subdomain_enum(target: str) -> set[str]:
    ...
    if rc != 0:
        return set()
    out_dir = REPO_ROOT / "engagements" / "runtime" / os.environ.get("ENGAGEMENT_ID", "default-engagement") / "recon" / target / "subdomains"
...
def configure_proxy_from_args(args: argparse.Namespace) -> None:
    ...
    if configured:
        ...
        say(color("  Tip: requests will appear in Burp Proxy → HTTP history.", "dim"))


def cmd_recon(args: argparse.Namespace) -> int:
    target = args.target
    out_dir = REPO_ROOT / "engagements" / "runtime" / os.environ.get("ENGAGEMENT_ID", "default-engagement") / "recon" / target
    resolved = out_dir.resolve()
    safe = (REPO_ROOT / "engagements").resolve()
    ...
    say()
    say(f"  Next: {color('bughunt classify <url>', 'bold')} for fast pattern-match, or {color('/hunt <target>', 'bold')} in Claude Code for full LLM-driven hunting")
    return 0
...
def write_recon_summary(target: str, subs: set[str], resolved: dict, live: list, out: Path):
    lines = [f"# Recon — {target}",
             "",
             f"_Generated by `bughunt recon {target}` at {datetime.datetime.now().isoformat(timespec='seconds')}._",
             "",
    ...
    lines += [
        ...
        "",
        "- For each live host, run `bughunt classify https://<host>/<path>?<params>` to surface attack candidates.",
...
URL_PATTERN_TO_SKILLS = [
    ...
    (r"/(login|signin|signup|register|forgot|reset)", ["hunt-auth-bypass", "hunt-ato", "hunt-brute-force"]),
    ...
    (r"/(2fa|mfa|otp|verify)", ["hunt-mfa-bypass", "hunt-brute-force"]),
    ...
    (r"/[?&].*?=\w+&.*?=", ["hunt-http-param-pollution"]),
    (r"/service(\?wsdl|\.asmx|\.svc)|/axis2/|/cxf/", ["hunt-soap"]),
    (r"/_vti_bin/.*\.asmx", ["hunt-soap", "hunt-sharepoint"]),
]
...
def cmd_classify(args: argparse.Namespace) -> int:
    ...
    if not result["matches"]:
        ...
        say(f"    {color('bughunt recon <target>', 'bold')} to map attack surface first.")
        return 1
    say()
    ...
    say("  1. Read the Pattern Library docs for each matched skill")
    say("  2. Pick the highest-confidence attack from the Pattern Library")
    say("  3. Apply OOB-Or-It-Didn't-Happen Gate before claiming success")
    say(f"  4. {color('bughunt triage <finding.md>', 'bold')} once you have a candidate finding")
    say()
    say(color("Need richer context? In a Claude Code conversation:", "dim"))
    say(color("   /hunt <url>     — full hunt-dispatch routing with LLM judgment", "dim"))
    say(color("   /chain          — build A→B→C exploit chains", "dim"))
    return 0
...
def cmd_triage(args: argparse.Namespace) -> int:
    ...
    if not fail_qs:
        ...
        say(color("Eligible for report drafting via `bughunt report <finding.md>`.", "dim"))
        return 0
...
# ============================================================
# ============================================================
# validate — run browser_driver.py on a finding
# ============================================================
def cmd_validate(args: argparse.Namespace) -> int:
    """Navigate to a finding URL via browser_driver.py and capture evidence."""
    finding_path = Path(args.finding)
    if not finding_path.exists():
    ...
    url_match = re.search(r"(?i)(?:URL|Endpoint|Target):\s*(\S+)", finding_text)
    if not url_match:
        print("ERROR: No URL found in finding", file=sys.stderr)
        return 1

    url = url_match.group(1)
    driver = str(REPO_ROOT / "scripts" / "browser_driver.py")
    ...
    if lib_path.exists():
        ...
        if str(lib_path) not in paths:
            paths.insert(0, str(lib_path))
        env["LD_LIBRARY_PATH"] = ":".join(paths)

    print(f"[bughunt] Navigating to {url} via browser_driver.py...", flush=True)

    result = subprocess.run(
        [py, driver, "navigate", url, "--wait=3"],
        capture_output=True, text=True, timeout=60, env=env,
    )
    if result.returncode != 0:
        print(f"[bughunt] Navigation FAILED: {result.stderr[:300]}", file=sys.stderr)
        return 1

    evidence_dir = Path("evidence")
    evidence_dir.mkdir(parents=True, exist_ok=True)
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    safe_name = "".join(c for c in args.finding if c.isalnum() or c in "-_")[:30]
    png_path = evidence_dir / f"validate-{safe_name}-{timestamp}.png"

    result = subprocess.run(
        [py, driver, "screenshot"],
        capture_output=True, text=True, timeout=30, env=env,
    )
    if result.returncode == 0 and "---IMAGE---" in result.stdout:
        b64_data = result.stdout.split("---IMAGE---")[1].split("---END IMAGE---")[0].strip()
        import base64
        png_path.write_bytes(base64.b64decode(b64_data))
        print(f"[bughunt] Screenshot saved to {png_path}", flush=True)
    ...
    if result.returncode == 0:
        try:
            ...
            cookie_path = evidence_dir / f"cookies-{safe_name}-{timestamp}.json"
            cookie_path.write_text(json.dumps(cookies, indent=2))
            print(f"[bughunt] {len(cookies)} cookies saved to {cookie_path}", flush=True)
        except json.JSONDecodeError:
            pass

    print("[bughunt] Validation complete. Update finding with evidence if confirmed.", flush=True)
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        description=("claude-bughunter CLI — terminal-native deterministic runner.\n"
                     ...
                     "in Claude Code are PRIMARY. Use bughunt for CI/CD, scripted runs, "
                     "deterministic verification, or when not in a Claude Code session."),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            Examples:
              bughunt recon hackerone.com
              bughunt recon target.com --burp                  # route via Burp proxy
              bughunt classify "https://api.target.com/v1/users/42?next=https://evil.com"
              bughunt triage findings/idor-2026-05-15.md
              bughunt report findings/idor-2026-05-15.md --platform bugcrowd --out draft.md
            ...
            See docs/bughunt-cli.md for the "when to use which" matrix.
            """),
    )
    ...
    def _add_proxy_args(p):
        ...
        p.add_argument("--browser", action="store_true",
                       help="Use browser_driver.py for validation instead of curl/Burp")
    ...
    p_validate = sub.add_parser("validate", help="validate a finding with browser_driver.py")
    p_validate.add_argument("finding", help="path to finding markdown file")
    p_validate.add_argument("--headless", action="store_true",
                            help="Run browser headless (default: headed)")
    _add_proxy_args(p_validate)
