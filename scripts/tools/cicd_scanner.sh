...
# Build sisakulint command (array-based — safe, no eval)
SISAKULINT_CMD=(sisakulint -remote "$TARGET" -D "$DEPTH" -l "$LIMIT" -p "$PARALLEL")
[ -n "$RECURSIVE" ] && SISAKULINT_CMD+=(-r)

log_info "Running: sisakulint -remote \"$TARGET\" -D $DEPTH -l $LIMIT -p $PARALLEL${RECURSIVE:+ -r}"
echo ""

# Run sisakulint and capture output (don't fail on non-zero exit — findings cause exit 1)
"${SISAKULINT_CMD[@]}" 2>&1 | tee "$SCAN_RESULTS" || true
