...
from tools.auth_session import add_cli_args, session_from_args  # noqa: E402
...
RECON_DIR = os.path.join(BASE_DIR, "engagements", "recon")
...
def setup_wordlists():
    """Symlink local wordlists for fuzzing."""
    import glob as pyglob
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    LOCAL_WORDLISTS = os.path.join(SCRIPT_DIR, "..", "..", "wordlists")
    os.makedirs(WORDLIST_DIR, exist_ok=True)

    for f in pyglob.glob(os.path.join(LOCAL_WORDLISTS, "*.txt")):
        name = os.path.basename(f)
        link = os.path.join(WORDLIST_DIR, name)
        if os.path.exists(link):
            continue
        os.symlink(os.path.abspath(f), link)
        log("ok", f"Linked {name}")
