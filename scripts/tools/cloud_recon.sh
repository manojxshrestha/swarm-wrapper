...
# cloud_enum is installed via install.sh at $TOOLS_DIR/cloud_enum/ with a venv
cloud_enum_path="$TOOLS_DIR/cloud_enum/cloud_enum.py"
...
if [ -n "$KEYWORD" ] && [ "$S3_ONLY" = "0" ]; then
  ...
  if [ -f "$cloud_enum_activate" ] && [ -f "$cloud_enum_path" ]; then
    ...
    (source "$cloud_enum_activate" && python3 "$cloud_enum_path" \
      -k "$KEYWORD" -t 5 --disable-aws-disk \
  ...
  elif _have cloud_enum; then
    log "cloud_enum sweep across AWS/Azure/GCP (PATH)..."
