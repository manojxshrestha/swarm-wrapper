...
```bash
./scripts/tools/param_extract.sh target.com   # Phase 1: find XSS candidates
./scripts/tools/auto_xss.sh target.com         # Phase 2: automated + manual payloads
...
12. **Validate with headed browser** — Always confirm in a real browser before reporting. Many payloads work in Burp but not in Chrome due to XSS auditors or browser parsing differences. Use the headed browser for automated validation:

    ```bash
    # Validate reflected XSS
    headed swarm-browser \
    ...
    If the agent confirms the payload fires, capture evidence with ``.
...
3. **Always use headed browser for real browser validation** — curl/Burp confirmation is insufficient for XSS. The headed browser navigates a real browser (Chrome/Firefox) to the vulnerable URL and checks whether JavaScript actually executes. This catches:
