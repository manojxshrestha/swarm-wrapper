...
./scripts/tools/phase-recon.sh $TARGET

# Or run individual phases:
./scripts/tools/subdomain_enum.sh $TARGET    # Phase 0: passive subdomain enum
./scripts/tools/phase-recon.sh $TARGET         # Phase 1: multi-engine crawl (or run individually: web_waymore.sh, web_gospider.sh, web_katana.sh)
...
# Skip slow steps:
./scripts/tools/phase-recon.sh $TARGET --skip dns,github,vhost,dir,zone
```
...
| 1 | `phase-recon.sh` | waymore + gospider + katana → uro | live crawled URLs |
