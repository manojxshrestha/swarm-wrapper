...
> See `scripts/tools/subdomain_enum.sh`, `scripts/tools/dns_bruteforce.sh`, `scripts/tools/phase-recon.sh`.
...
bash $HOME/swarm/scripts/tools/subdomain_enum.sh "$D"
...
function Get-Subs {
  ...
  "crt.sh down — pivot to subdomain_enum.sh: bash $HOME/swarm/scripts/tools/subdomain_enum.sh $D" | Out-Host
  return @()
}
```
...
bash $HOME/swarm/scripts/tools/subdomain_enum.sh target.example   # subfinder + assetfinder + findomain → dnsx → httpx
bash $HOME/swarm/scripts/tools/dns_bruteforce.sh target.example    # puredns brute-force with validated resolvers
