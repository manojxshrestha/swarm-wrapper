...
"$HOME/.local/bin/msftrecon/venv/bin/python3" \
  "$HOME/.local/bin/msftrecon/msftrecon/msftrecon.py" \
  -d sister-brand-school.example
# Or via phase-intel.sh which wraps the venv invocation:
#   scripts/tools/phase-intel.sh -t client.example
```
...
When ROPC returns AADSTS53003, you've proven the password. To prove it across BOTH auth paths (and capture Microsoft's CA-block page as evidence), walk SAML SSO via headed browser:
