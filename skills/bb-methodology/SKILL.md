...
| 5 | If Cloudflare detected | Redirect 80% effort to API subdomain; use headed browser for CF pages |
...
**Invocation via venv (installed at `$HOME/.local/bin/<tool>/`):**
```bash
# Scopify — scope analysis
"$HOME/.local/bin/Scopify/venv/bin/python3" \
  "$HOME/.local/bin/Scopify/scopify.py" -c "Company Name"

# Spoofy — SPF/DMARC spoof check
"$HOME/.local/bin/Spoofy/venv/bin/python3" \
  "$HOME/.local/bin/Spoofy/spoofy.py" -d target.example

# cloud_enum — cloud bucket enumeration
"$HOME/.local/bin/cloud_enum/venv/bin/python3" \
  "$HOME/.local/bin/cloud_enum/cloud_enum.py" -k keyword -t 20 -qs
``` |
...
- `search_wstg()` — WSTG test cross-reference
...
| 3 | Cross-reference severity against WSTG technique guides | `search_wstg()` |
