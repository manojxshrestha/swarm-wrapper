...
SAML/SSO flows are inherently browser-based. Use the headed browser to validate redirect flows, token acceptance, and cross-IdP behavior:
...
3. **Always use headed browser for SAML flow validation** — SSO redirect flows cannot be fully tested with curl/Burp alone. The browser preserves IdP cookies, follows redirects, and executes JavaScript that server-side testing misses.
