...
```
**Server accepts** (prefix matches `https://acme.example/`). But the browser parses this URL as a path under `acme.example` — the code does NOT reach `evil.example.com`. Server-side flaw confirmed; browser-side bypass requires a no-trailing-slash prefix to be exploitable.

#### Attack B — userinfo `@` bypass (Phase 3.1 browser-execution analysis)
...
**Phase 3.1 — browser-execution verification shows the "code reaches attacker" claim does not apply against THIS specific lab.** When Playwright/Chromium navigates to `https://acme.example/@evil.example.com/?code=...`, the WHATWG URL parser (which all modern browsers use) reads it as:

- scheme: `https`
- host: `acme.example` ← stops at the FIRST `/` after `://`
- path: `/@evil.example.com/`

The `@` is just a path character because there's already a `/` between `acme.example` and `@`. **The auth code lands at `acme.example`, NOT `evil.example.com`.** The RFC-3986-userinfo bypass chain is blocked at the browser-parse layer under this lab configuration (trailing-slash prefix).
...
See `docs/verification/phase3-browser-execution.md` Test 29 for the live browser trace confirming the correct attack shape against a no-trailing-slash prefix.

**Operational rule (now in `hunt-oauth`):** server-side prefix-match flaw is necessary but NOT sufficient for browser-level ATO. Always headless-test the final navigation before writing the finding as "ATO chain via @-userinfo bypass". The `@`-userinfo bypass is exploitable against no-trailing-slash prefixes; trailing-slash prefixes block the chain at the browser-parse layer.
