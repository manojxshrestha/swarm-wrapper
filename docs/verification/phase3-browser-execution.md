# Verification — Phase 3.1: Browser-execution harness

> Closes the "source-verified only" gap from Phase 2F. Real headless Chromium executes the payloads and we read the DOM / nav chain to confirm. Three browser-side tests, three honest outcomes.


> **Legacy note:** This verification was originally executed with Playwright directly.
> The project now uses **browser_driver.py** (`scripts/browser_driver.py`) as its browser automation layer.
> For new verifications, use the MCP browser tools (browser_login, browser_screenshot, etc.) or `scripts/browser_driver.py` directly.
...
pip install flask  # (or: `uv pip install --python .venv/bin/python playwright` for new tests)
# Playwright install: playwright install chromium (legacy, for this specific harness)
python target_app.py &   # Flask target on :58020
python harness.py         # Run the original Playwright tests
```
...
## Test 29 — OAuth `@`-userinfo bypass — browser-execution analysis
...
For this specific lab (trailing-slash prefix), server-side prefix-check accepted the URL but the browser never reached `evil.attacker.example` — the `@`-userinfo bypass chain is blocked at the browser parse layer under these conditions.
...
**Phase 2F Test 14 Attack B is updated to reflect this finding.**

---

## Phase 2F update → `docs/verification/phase2f-ssti-oauth-fileupload.md`

Add to Phase 2F's Attack B writeup:

> **Phase 3.1 finding:** Browser-execution verification (`docs/verification/phase3-browser-execution.md` Test 29) confirms that against a `https://acme.example/` (trailing-slash) prefix, browsers parse the `@` as a path character — the auth code lands at `acme.example`, not at the attacker. The `@`-userinfo bypass requires the prefix to have NO trailing slash (or for the server check to be substring-match instead of `startswith`). The vulnerability class is real; exploitation requires the no-trailing-slash server configuration.
>
> Operationally: when the server-side prefix-check passes against an `@`-shaped URL, run a headed browser check of the actual browser navigation BEFORE writing the finding as ATO-chain — the server-side accept and the browser-side navigation are different gates.
...
| 29 | OAuth `@`-userinfo browser navigation | **Real vulnerability confirmed against no-slash prefix; requires no-trailing-slash server config.** |

**3 / 3 honest outcomes.** Two clean execution PASSes + one Phase 2F finding refinement with actionable operational guidance.
