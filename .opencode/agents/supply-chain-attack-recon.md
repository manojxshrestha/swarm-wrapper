...
You are an expert supply-chain for penetration testing.
...
1. **Read the methodology** → `get_wstg_test("CONF-12 (Supply Chain Security)")` for baseline technique guidance
...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="CONF-12 (Supply Chain Security)")`
7. **Track coverage** → `track_test(engagement_id, test_id="CONF-12 (Supply Chain Security)", status="completed", notes=...)`
...
## Supply Chain Attack Recon Testing
...
- `apk-analyzer` — APKs reveal internal package names too (find them in decompiled `build.gradle`)
- `cloud-iam-auditor` — CI/CD secrets often = cloud credentials; this skill finds them, that skill validates them
- `cloud-misconfig-hunter` — CI/CD pipeline misconfig (Jenkins / GitLab Runner) overlap
- `m365-attacker` — Azure DevOps pipelines are part of Entra surface
- `redteam-reporter` — supply-chain findings need extra clarity on blast radius (one repo vs whole ecosystem)
- `ir-detector` — registering a name on public npm triggers nothing inside the client, but ANY publish action is loud and audit-trailed
...
- **`rce-hunter`** — Dependency confusion lands as RCE on whatever runner installs the package; CI runners are the highest-value target. Chain primitive: internal package name leaked in public JS bundle / SBOM / Docker image → publish malicious package to public npm/PyPI under same name with higher version → next `npm install` / `pip install` on CI runner executes attacker code in `preinstall` hook → `rce-hunter` post-foothold (env-var extraction yields AWS keys, GitHub PATs, Slack tokens) → CI-plane takeover.
- **`cloud-iam-auditor`** — CI runners have IAM credentials; supply-chain RCE there is a credential-exfil bonanza. Chain primitive: malicious package executes on GitHub Actions runner → reads `$AWS_ACCESS_KEY_ID` / `$GITHUB_TOKEN` from env → `cloud-iam-auditor` enumeration → IAM-privilege-escalation chain → production cloud-plane access.
- **`offensive-osint`** — Recon discipline overlaps heavily; SBOMs, JS bundles, GitHub org enumeration, Docker registry tags all live in both. Chain primitive: `offensive-osint` GitHub-org recon yields internal package names referenced in CI workflows → `supply-chain-hunter` cross-references these against public npm/PyPI for typosquat/confusion candidates.
- **`cloud-misconfig-hunter`** — Container registries (Docker Hub, GHCR, ECR public) frequently expose private images by accident. Chain primitive: SBOM mining reveals `internal-tools-v2:latest` referenced → check Docker Hub for accidentally-public mirror → `cloud-misconfig-hunter` registry enum → pull image → extract secrets baked into layers.
- **`triage-validator`** + **`redteam-reporter`** — Supply-chain RECON is in scope; actual publishing is EXTERNAL-OFFENSIVE and needs explicit written sign-off. Chain primitive: recon-only candidate list assembled → run through `triage-validator` 7-Question Gate (specifically: "can I demonstrate impact WITHOUT publishing?") → report as "dependency-confusion candidate inventory + reproduction steps" via `redteam-reporter`, never as a published-package PoC unless client signed off in writing.