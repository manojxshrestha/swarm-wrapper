...
You are an expert bugcrowd for penetration testing.
...
1. **Read the methodology** → `get_wstg_test("All phases (Bugcrowd Reporting)")` for baseline technique guidance
...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="All phases (Bugcrowd Reporting)")`
7. **Track coverage** → `track_test(engagement_id, test_id="All phases (Bugcrowd Reporting)", status="completed", notes=...)`
...
This skill encodes patterns that apply specifically to Bugcrowd's submission flow. For the generic per-platform templates (HackerOne / Bugcrowd / Intigriti / Immunefi report bodies), use the `report-writing` skill. For the 7-Question Gate before deciding to report at all, use `triage-validator`.
...
|---|---|
| "Should I report this finding at all?" | `triage-validator` (7-Question Gate) |
| "What's the report body template for Bugcrowd?" | `report-writing` |
| "What VRT do I pick?" | This skill (`bugcrowd-reporter`) §1 |
...
This skill is small and focused. It does NOT duplicate content from `report-writing` (per-platform templates, CVSS scoring, downgrade counters, 60-second pre-submit checklist) — load both skills together when filing a Bugcrowd report. The two skills' content composes naturally because their boundaries are clean: `report-writing` is the body templates, `bugcrowd-reporter` is the program-specific tactics layered on top.

---

## Related Skills & Chains

- **`report-writing`** — Always load this skill alongside `bugcrowd-reporter`. Workflow primitive: `report-writing` provides the body skeleton; this skill layers VRT selection + severity-request paragraph + OOS-clause rebuttals on top of that skeleton.
- **`triage-validator`** — When deciding if a Bugcrowd-bound finding will pass triage. Workflow primitive: 7Q gate runs first; only validated findings reach this skill for VRT mapping.
