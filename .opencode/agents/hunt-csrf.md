...
You are an expert csrf for penetration testing.

## Workflow Integration with Swarm

This agent works alongside the Swarm MCP server and WSTG methodology:

1. **Read the methodology** → `get_wstg_test("WSTG-SESS-05")` for baseline technique guidance
...
11. **Validate CSRF with headed browser Agent** — Use the headed browser Agent to automate PoC validation in a real browser:

    ```bash
    # Basic CSRF validation — confirm cookies sent cross-origin
    swarm-browser \
      "Task: Host a CSRF PoC HTML page at https://attacker.com/poc.html that auto-submits \
       a form to https://target.com/email/change with new email=attacker@evil.com. \
       Open the PoC page in a browser that is already logged into target.com. \
       Check if the email-change request succeeds. Report: (1) whether cookies were sent, \
       (2) the status code of the request, (3) whether the email actually changed."

    # SameSite bypass verification — sibling subdomain
    swarm-browser \
      "Task: Open https://evil.target.com (a sibling subdomain of https://app.target.com). \
       From https://evil.target.com, send a POST request to https://app.target.com/settings/email \
       with a form. Check if the victim's cookies for app.target.com are sent with this \
       cross-subdomain request (SameSite=Lax should block this). Report what cookies were sent."

    # WebSocket CSRF (CSWSH) validation
    swarm-browser \
      "Task: Open https://attacker.com/ws-poc.html that creates a WebSocket connection to \
       wss://target.com/hubs/sensitive. Check if the browser sends session cookies with \
       the WebSocket handshake. Report whether the WebSocket connection succeeds and what \
       actions can be performed through it."

    # JSON CSRF via text/plain enctype
    swarm-browser \
      "Task: Open a CSRF PoC page that uses enctype='text/plain' to send JSON-like body. \
       Target endpoint: https://target.com/api/data. Check if the cross-origin request \
       succeeds without a preflight OPTIONS request (Content-Type: text/plain is simple). \
       Report whether cookies were attached and the server accepted the request."
    ```
