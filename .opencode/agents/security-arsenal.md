...
You are an expert security-arsenal for penetration testing.
...
1. **Read the methodology** → `get_wstg_test("All phases (Tool Reference)")` for baseline technique guidance
...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="All phases (Tool Reference)")`
7. **Track coverage** → `track_test(engagement_id, test_id="All phases (Tool Reference)", status="completed", notes=...)`
...
gf cmdi         # Command injection parameters
gf crlf         # CRLF injection parameters
gf csti         # Client-side template injection
gf img-traversal # Image path traversal
gf injection    # General injection parameters
gf interestingEXT # Interesting file extensions
gf interestingparams # Interesting parameters
gf interestingsubs # Interesting subdomains
gf jsvar        # JS variable patterns
gf rfi          # Remote file inclusion
...
- **`xss-hunter`** / **`ssrf-hunter`** / **`sqli-hunter`** / **`ssti-hunter`** / **`idor-hunter`** — When a hunter is actively testing a parameter and needs payloads. Workflow primitive: this skill is the payload library those hunt-* skills reach for; the hunt-* skill identifies the sink, this skill provides the syntax.
- **`triage-validator`** — When deciding if a finding is reportable at all. Workflow primitive: the "Always Rejected" and "Conditionally Valid — Requires Chain" tables in both skills must agree; `triage-validator` runs the 7-Question Gate, this skill provides the chain-required mapping used by Q7.
