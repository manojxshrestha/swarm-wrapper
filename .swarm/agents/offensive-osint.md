...
You are an expert offensive-osint for penetration testing.
...
1. **Read the methodology** → `get_wstg_test("INFO-01 (Search Engine Recon)")` for baseline technique guidance
...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="INFO-01 (Search Engine Recon)")`
7. **Track coverage** → `track_test(engagement_id, test_id="INFO-01 (Search Engine Recon)", status="completed", notes=...)`
...
## Offensive Osint Testing

# Offensive OSINT — External Red-Team Arsenal

> **v3.0** — Refactored 2026-05-02 from a 4,168-line monolith into a lean SKILL.md (~400 lines) plus 15 modular reference files in `references/`. Detail content loads on demand — the agent reads only the reference files relevant to the current task.
...
| Signal | Points |
|---|---|
| Package reverse-DNS matches target domain (e.g., `com.[target].android` ⟂ `[target]`) | +40 |
...
19. *"Audit the target's SPF/DMARC for spoof feasibility."* → §16.14.
...
- **`subdomain-hunter`** — When this skill's subdomain enumeration finds stale CNAMEs. Workflow primitive: subdomains discovered via §27 / `references/recon-stack.md` get auto-routed to `subdomain-hunter` for takeover validation.
- **`cloud-misconfig-hunter`** — When this skill's cloud-bucket enum surfaces listable buckets / Firebase / actuator endpoints. Workflow primitive: §39 attack-path hints (listable bucket, /actuator/env, open Elasticsearch) hand off to `cloud-misconfig-hunter` for exploitation.
- **`m365-attacker`** / **`okta-attacker`** — When identity-fabric fingerprinting finds Entra/Okta. Workflow primitive: `references/identity-fabric.md` fingerprints the IdP; matched platform skill (loaded by `hunt-dispatch`) takes over for active enumeration.