...
OOB means: a Burp Collaborator domain, an `interactsh-client` listener, `swarm-oob` (scripts/tools/oob_listener.sh), a canarytoken, or any DNS+HTTP receiver you control that confirms the server actually made an outbound network connection on your behalf.
...
- **Headless browsers** (headed browser Agent) used for screenshots/PDF — extremely high value
