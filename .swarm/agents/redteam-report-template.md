...
You are an expert redteam for penetration testing.
...
1. **Read the methodology** → `get_wstg_test("All phases (Red Team Reporting)")` for baseline technique guidance
2. **Check related prompt** → read `prompts/templates/report-template.md` for Swarm-specific workflow
...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="All phases (Red Team Reporting)")`
7. **Track coverage** → `track_test(engagement_id, test_id="All phases (Red Team Reporting)", status="completed", notes=...)`
...
## Redteam Report Template Testing
...
Do NOT use for:
- Bug-bounty platform submissions (use `report-writing` / `bugcrowd-reporter` instead)
...
- **Confirmed; patched mid-engagement** — was reproducible, client patched during the test window (still ship the finding — see `ir-detector`)
...
If a finding was confirmed and then stopped working, that is almost always a CLIENT PATCH, not a finding-was-false. The correct response is "Confirmed; patched mid-engagement" with timestamps showing when it broke. See `ir-detector`.
...
- `ir-detector` — informs the "patched mid-engagement" status pattern
- `evidence-hygiene` — informs screenshot redaction discipline
- `m365-attacker`, `vpn-attacker`, etc. — each provides finding-templates specific to its attack surface
...
- **`triage-validator`** — This template ingests findings that have ALREADY passed the 7-Question Gate. Engagement flow: every finding through `triage-validator` first → only validated findings → `redteam-reporter` packaging. Skipping triage produces a deliverable padded with informational noise that erodes client trust.
- **`evidence-hygiene`** — The DOCX with 16 embedded screenshots only works if evidence was captured systematically throughout the engagement. Engagement flow: `evidence-hygiene` discipline at session start → timestamped, organized screenshot folder → `redteam-reporter` consumes that folder to populate Evidence blocks.
- **`redteam-mindset`** — The Subject / Observations / Description / Impact / Recommendation / PoC structure assumes the operator already thinks like a red-teamer (impact-first, blast-radius framing). Engagement flow: `redteam-mindset` loaded at engagement start → findings collected with red-team framing baked in → `redteam-reporter` produces deliverable without rewriting every Impact section.
- **`ir-detector`** — Defensive-action findings (SOC patches mid-test, new IPS rules deployed, account lockouts triggered by external attacker) are first-class findings in red-team deliverables. Engagement flow: `ir-detector` captures behavior-change events → each becomes its own Subject in the deliverable, framed as "client capability observation" not as "bug we missed."
- **`report-writing`** + **`bugcrowd-reporter`** — Bug-bounty platform reports use DIFFERENT structure (one finding per submission, platform-specific severity scoring, OOS-clause counters). Engagement flow: if engagement mode is `bug-bounty` per project memory → use `report-writing` / `bugcrowd-reporter` instead. This template is ONLY for external red-team / enterprise client deliverables.